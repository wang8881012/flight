document.addEventListener('DOMContentLoaded', () => {
    ...
    let bookedSeats = [];        // 儲存後端回傳：已被選走的座位
    let currentClassType = null; // 從後端設定：目前選擇的艙等

    fetch('../api/booking/seats_&_addons/get_seats.php')
        .then(res => res.json())
        .then(data => {
            bookedSeats = data.booked || [];
            currentClassType = data.class_type;
            drawMap();
        })
        .catch(err => console.error("無法取得座位資料", err));
    ...
    const img = new Image();
    img.src = '../assets/images/flight_seat.svg';
    img.onload = () => {
    ...
    function isSeatInCurrentClass(cls) {
        if (!currentClassType) return true;
        const classMap = {
            business: 'bs',
            economy: 'st',
            premium: 'pro',
        ...
        return classMap[currentClassType] === cls;
    }
    ...
    canvas.addEventListener('click', e => {
        ...
        seatModalEl.addEventListener('hidden.bs.modal', () => {
            setTimeout(() => {
                nextBtn.focus(); // 延遲一點點再 focus
            }, 100); // Bootstrap 通常動畫時間為 150ms
        });
    });
    ...
    nextBtn.addEventListener('click', () => {
        ...
        const postData = {
            ...
            baggage: {
                weight: document.getElementById('checked_baggageWeight').value,
                price: document.getElementById('checked_baggagePrice').value
            }
        };
        //console.log(postData)

        fetch('../api/booking/seats_&_addons/save_addons.php', {
            method: 'POST',
